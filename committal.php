...
<div class="main">
  ...
  <div class="content">
    <div class="content_resize">
      <div class="mainbar">
          <div class="article">
          <h2><span>Register</span>  New Committal Prisoner Details</h2>
          <div class="clr"></div>
		   <p style="margin-left:90px;">You will use this form to register all your details for Committal Prisoners</p>
		  </div>
          <form action="committal.php" method="post" enctype="multipart/form-data" id="sendemail" style="margin-left:90px;">
            <ol>
...
<option value="" >------------------------Select District--------------------------</option>
	  ...
	  <li>
	    ...
	    <input type="submit" class="form_submit" value="Register Committal Prisoner" style="width:450px;height:30px; margin-right: 48px; margin-top:5px"  name="save" />
        ...
        if(isset($_POST["save"]) && !empty($_POST["convict"]) && !empty($_POST["fname"]) && !empty($_POST["lname"]) && !empty($_POST["son"]) && !empty($_POST["gender"]) && !empty($_POST["dob"]) && !empty($_POST["offence"]) && !empty($_POST["prisoner"]) && !empty($_POST["court"]) && !empty($_POST["ms"]) && !empty($_POST["adate"]) && !empty($_POST["cdate"]) && !empty($_POST["description"]) && !empty($_POST["kin"]) && !empty($_POST["tel"]) && !empty($_POST["district"]) ) {
		   ...
		   $prisoner=$_POST["prisoner"];
			...
			$image = addslashes(file_get_contents($_FILES['image']['tmp_name']));
                ...
                $insert="insert into committal (CommittalNo,FirstName,SecondName,SonOf,Gender,DOB,Offence,TypeOfPrisoner,Court,MaritalStatus,AdmissionDate,ConvictionDate,Description,NextOfKin,Telephone,District,Photo)values('$convict','$fname','$lname','$son','$gender','$dob','$offence','$prisoner','$court','$ms','$adate','$cdate','$description','$kin','$tel','$district','$location')";
