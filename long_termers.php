...
<div class="main">
  ...
  <div class="content">
    <div class="content_resize">
      <div class="mainbar">
        <div class="article">
          <h2><span>Register</span> Long Termer Prisoner</h2>
          <div class="clr"></div>
		   <p style="margin-left:90px;">You will use this form to register all your details for Long Termer Prisoners</p>
		  </div>
          <form action="long_termers.php" method="post" enctype="multipart/form-data" style="margin-left:90px;" >
	...
	<input type="text" class="form_input" style="width:450px;height:25px" class="textbox" placeholder="Enter Sentence" name="sentence" required  />
	    ...
	    <li>
		  <label>Photo :</label>
            ...
            <li>
	    <input type="submit" id="imageField" src="images/submit.gif" class="send" value="Register Long Termer Prisoner" style="width:450px;height:30px; margin-right: 48px; margin-top:5px"  name="save" />			 
            </li>
        ...
        if(isset($_POST["save"]) && !empty($_POST["convict"]) && !empty($_POST["fname"]) && !empty($_POST["lname"]) && !empty($_POST["son"]) && !empty($_POST["gender"]) && !empty($_POST["dob"]) && !empty($_POST["offence"]) && !empty($_POST["prisoner"]) && !empty($_POST["court"]) && !empty($_POST["ms"]) && !empty($_POST["adate"]) && !empty($_POST["cdate"]) && !empty($_POST["edate"]) && !empty($_POST["ldate"]) && !empty($_POST["description"]) && !empty($_POST["kin"]) && !empty($_POST["tel"]) && !empty($_POST["district"]) && !empty($_POST["sentence"]) ) {
		   ...
		   $prisoner=$_POST["prisoner"];
		    ...
		    $image = addslashes(file_get_contents($_FILES['image']['tmp_name']));
				...
				header("Location:long_termers.php");
