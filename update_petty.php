...
<div class="main">
  ...
  <div class="content">
    <div class="content_resize">
      <div class="mainbar">
          <div class="article">
          <h2><span>Update</span> Petty Prisoner Details</h2>
          <div class="clr"></div>
		   <p style="margin-left:90px;">You will use this form to Update all your registered details for Petty Offender Prisoners Details.</p>
		   <p>You will use this form to Update all your registered details for Petty Offender Prisoners Details.</p>
		   </div>
		  ...
		  <?php
               ...
               $id=$_GET["id"]; 
                ...
                $select="select * from petty where Id=$id";
        ...
        ?>
		  ...
		  <form action="editpetty.php?id=<?php echo $row['Id']; ?>" enctype="multipart/form-data" method="post" style="margin-left:90px;">
   ...
   <div id="tab4" class="tabcontent">
      <!--  <h3>Booking Form for  my clients</h3>  -->
        <div class="form">
			...
			<li>
            <label> Petty Offender Number :</label>
            <input type="text" class="form_input" style="width:450px;height:25px" class="textbox" placeholder="Enter Petty Offender Number" name="convict" required onKeyUp="checkInput(this)" value="<?php echo $row['PettyNo']; ?>"   /> 
		</li>
            <li>
...
<option><?php echo $row['District'];?></option>
  ...
  <li>
	    ...
	    <input type="submit" class="form_submit" value="Update Petty Offender Prisoner Details" style="width:450px;height:30px; margin-right: 48px; margin-top:5px"  name="save" />
 ...
 <ul class="sb_menu_coms">
	  ...
	  <?php
        if(isset($_POST["save"]) && !empty($_POST["petty"]) && !empty($_POST["fname"]) && !empty($_POST["lname"]) && !empty($_POST["son"]) && !empty($_POST["gender"]) && !empty($_POST["dob"]) && !empty($_POST["offence"]) && !empty($_POST["prisoner"]) && !empty($_POST["court"]) && !empty($_POST["ms"]) && !empty($_POST["adate"]) && !empty($_POST["description"]) && !empty($_POST["kin"]) && !empty($_POST["tel"]) && !empty($_POST["district"]) ) {
           $petty=$_POST["petty"];
		   $fname=$_POST["fname"];  
           $lname=$_POST["lname"];
		   $son=$_POST["son"];
		   ...
		   $prisoner=$_POST["prisoner"];
			...
			$image = addslashes(file_get_contents($_FILES['image']['tmp_name']));
                ...
                $insert="insert into petty (PettyNo,FirstName,SecondName,SonOf,Gender,DOB,Offence,TypeOfPrisoner,Court,MaritalStatus,AdmissionDate,Description,NextOfKin,Telephone,District,Photo)values('$petty','$fname','$lname','$son','$gender','$dob','$offence','$prisoner','$court','$ms','$adate','$description','$kin','$tel','$district','$location')";
